...
class WikipediaParser(BaseWebParser):
    ...
    def __init__(self, urls):
        ...
        #Configurazione specifica per Wikipedia
    ...
    def extract_data(self, result) -> Dict[str, Any]:
            ...
            if result.success and result.html:
                # Applica la tua logica BeautifulSoup
    ...
    def extract_wikipedia_text(self, html: str) -> str:
        ...
        #Rimozione degli elementi inutili (tabelle, note, infobox, ecc)
        for selector in [
            "sup.reference", ".mw-references-wrap", ".reference",
            ".infobox", ".ambox", ".navbox", ".metadata", ".mw-editsection",
            "style", "script", ".hatnote", ".mw-empty-elt"
        ]:
            for tag in content.select(selector):
                tag.decompose()

        #Estraiamo gli elemenyi principali da mantenere
        ...
        for el in elements:
            if el.name in ('h2', 'h3'):
                ...
                #Escludiamo solo le sezioni di servizio finali
                if any(skip in lower for skip in [
                    "note", "references", "external links", "see also", "bibliography"
                ]):
                    continue
                ...
                # Convertiamo la tabella in markdown semplice
                table_md = self._table_to_markdown(el)
                if table_md and len(table_md.strip()) > 20:   # solo tabelle con contenuto significativo
                    text_parts.append(table_md)
        # Unione finale
        final_text = "\n\n".join(text_parts).strip()
        final_text = re.sub(r"\n{3,}", "\n\n", final_text)   # massimo due a capo consecutivi
        return final_text

    def _table_to_markdown(self, table) -> str:
        """Converte una tabella HTML semplice in formato Markdown."""
        ...
        # Aggiunge la riga di separazione dopo l'intestazione
        if markdown_rows:
